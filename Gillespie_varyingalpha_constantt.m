...
% algorithm. Including factors of decaying fitness values 'alphas' impacting R2 due to
% antivirals and dosing such antivirals at different moments in time t . 
...
mu_array = linspace(0,0.15,100) ;  % mutation rate(s). Can be a single float (one mutation rate) or an array.
...
% Start for-loop over error rates #########################################
    ...
    if length(mu_array) == 1    % if only one mutation rate:
        ...
        %title(['Stationary value of particle number with alpha =',num2str(alpha)],'Fontsize',16)
        ...
        suptitle(['Error catastrophe with Gillespie algorithm with inhibition factor \alpha =', num2str(alpha)])
    end
end
%% Functions 
