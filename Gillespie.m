...
a = 50e-5;                   % infection rate per day.
b = 0.4;                    % death/clearence rate of infected/immune cells per day.
...
mu_array = ones(1,1)*1e-6;  % mutation rate(s). Can be a single float (one mutation rate) or an array.
S = 1e3;                    % initial size of arrays
nIter_record = 100;         % record every 100 iterations
nIter_print = 1000;         % print every 1000 iterations
k = 1;                      % killing of infected cells by immune cells
...
statY = zeros(N_mu, L + 1);                    	% stationary value of particle number
titer = struct();
...
for k = 1:N_mu
    ...
    while t < T
         ...
         for iEpitope = existing_epitope % loop over unique epitopes 
             ...
             c = c + k*sum(I(Epitope2Strain_index))*Tcells(iEpitope);                                           % Killing: Cytotoxic T cells kill infected cells
             if c > rnd
