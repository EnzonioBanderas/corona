...
% algorithm. Constant alpha but dosed at different time t 
...
alpha = 0.7;
doseday_array = [0,1,2,3,4,5,6];
mu_array = linspace(0,0.1,10);
...
% Start for-loop over times  #########################################
for v = 1:length(doseday_array)
    days = doseday_array(v);
    disp(['Days after infection of antiviral injection = ', num2str(days)]) 
...
% Start for-loop over error rates #########################################
    ...
    if length(mu_array) == 1    % if only one mutation rate:
        ...
        suptitle(['Error catastrophe with Gillespie algorithm with injection day t =', num2str(days)])
    end
end
%% Functions 
